import requests
from flask import Flask, request, render_template

app = Flask(__name__)

@app.route('/html')
def html_page():
    return '''
    <h1>Hello</h1>
    <img src="data:image/jpeg;base64," alt="картина">
    '''

@app.route('/home')
def home_page():
    return render_template("file.html", login='zhan1234', age=16, name='zhan')

ls1 = ["1 блог", "2 блог", "3 блог"]
ls2 = ["Сегодня я начал изучать программирование. Мне интересно создавать свои сайты и приложения.", "Flask — удобный инструмент для создания веб-приложений на Python. Сейчас я учусь создавать страницы и передавать данные.", "Я создаю свой первый небольшой проект. Пока он простой, но со временем я хочу добавить больше функций."]
@app.route('/blog')
def blog_page():
    return render_template("file2.html", blog1=ls1, blog2=ls2)

app.run()

